...
namespace HSCore
{
    public static class NetDecks
    {
        ...
        public static List<Deck> DownloadDecks()
        {
            ...
            string message = $"TempoStormReader - Standard:{decks.Count(x => x.DeckType == DeckType.Standard)} - Wild:{decks.Count(x => x.DeckType == DeckType.Wild)} - Error:{decks.Count(x => x.IsError)} - Total:{decks.Count}";
            log.Info(message);
            ...
            message = $"HearthstoneTopDecksReader - Standard:{decks.Count(x => x.DeckType == DeckType.Standard)} - Wild:{decks.Count(x => x.DeckType == DeckType.Wild)} - Error:{decks.Count(x => x.IsError)} - Total:{decks.Count}";
            log.Info(message);
            _decks.AddRange(decks);
            ProgressChanged($"(2/6) Downloading Decks...     {decks.Count} added. Total:{_decks.Count}");
            reader = new ViciousSyndicateReader();
            decks = reader.GetDecks();
            message = $"ViciousSyndicateReader - Standard:{decks.Count(x => x.DeckType == DeckType.Standard)} - Wild:{decks.Count(x => x.DeckType == DeckType.Wild)} - Error:{decks.Count(x => x.IsError)} - Total:{decks.Count}";
            log.Info(message);
            _decks.AddRange(decks);
            ProgressChanged($"(3/6) Downloading Decks...     {decks.Count} added. Total:{_decks.Count}");
            reader = new MetabombReader();
            decks = reader.GetDecks();
            message = $"MetabombReader - Standard:{decks.Count(x => x.DeckType == DeckType.Standard)} - Wild:{decks.Count(x => x.DeckType == DeckType.Wild)} - Error:{decks.Count(x => x.IsError)} - Total:{decks.Count}";
            log.Info(message);
            _decks.AddRange(decks);
            ProgressChanged($"(4/6) Downloading Decks...     {decks.Count} added. Total:{_decks.Count}");
            reader = new DisgusedToastReader();
            decks = reader.GetDecks();
            message = $"DisgusedToastReader - Standard:{decks.Count(x => x.DeckType == DeckType.Standard)} - Wild:{decks.Count(x => x.DeckType == DeckType.Wild)} - Error:{decks.Count(x => x.IsError)} - Total:{decks.Count}";
            log.Info(message);
            _decks.AddRange(decks);
            ProgressChanged($"(5/6) Downloading Decks...     {decks.Count} added. Total:{_decks.Count}");
            reader = new HSReplayReader();
            decks = reader.GetDecks();
            message = $"HSReplayReader - Standard:{decks.Count(x => x.DeckType == DeckType.Standard)} - Wild:{decks.Count(x => x.DeckType == DeckType.Wild)} - Error:{decks.Count(x => x.IsError)} - Total:{decks.Count}";
            log.Info(message);
            _decks.AddRange(decks);
            ProgressChanged($"(6/6) Downloading Decks...     {decks.Count} added. Total:{_decks.Count}");
            log.Info($"Error in {_decks.Count(x => x.IsError)} decks: {string.Join(" ; ", _decks.Where(x => x.IsError).Select(x => x.Url))}");
